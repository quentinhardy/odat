...
class Unwrapper (OracleDatabase):
	...
	def __getSourceCode__ (self, objectName):
		'''
		returns souce code of the object objectName
		returns {'owner':'', 'sourceCode':''} or None if no result
		'''
		sourceCode = ""
		logging.info("Geeting the source code of the object named {0}".format(objectName))
		logging.debug("Sending this request: {0}".format(self.REQ_GET_SOURCE_CODE.format(objectName)))
		results = self.__execQuery__(query=self.REQ_GET_SOURCE_CODE.format(objectName), ld=['text', 'owner'])
		if results == []:
	...
	def unwrapRemotely(self, objectName):
		...
		'''
		sourceCode = self.__getSourceCode__(objectName)
